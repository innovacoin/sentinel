...
def has_innova_conf():
    ...
    valid_innova_conf = False

    # ensure innova_conf exists & readable
    #
    # if not, print a message stating that Innova Core must be installed and
    # configured, including JSONRPC access in innova.conf
    try:
        f = io.open(config.innova_conf)
        valid_innova_conf = True
    except IOError as e:
        print(e)

    return valid_innova_conf
...
def main():
    ...
    if not has_innova_conf():
        print("InnovaCore must be installed and configured, including JSONRPC access in innova.conf")
        sys.exit(1)
