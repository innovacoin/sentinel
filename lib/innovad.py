...
class InnovaDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some innovad methods
    ...
    @classmethod
    def from_innova_conf(self, innova_dot_conf):
        from innova_config import InnovaConfig
        config_text = InnovaConfig.slurp_config_file(innova_dot_conf)
        creds = InnovaConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = innovalib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = innovalib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = innovalib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_innovad_proto_version_with_sentinel_ping)
