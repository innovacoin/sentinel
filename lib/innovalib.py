...
def is_valid_innova_address(address, network='mainnet'):
    ...
    innova_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Innova address
        return False

    if (address_version != innova_version):
        return False
...
# shims 'til we can fix the innovad side
def SHIM_serialise_for_innovad(sentinel_hex):
    ...
    # superblock => "trigger" in innovad
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # innovad expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    innovad_hex = serialise(obj)
    return innovad_hex


# shims 'til we can fix the innovad side
def SHIM_deserialise_from_innovad(innovad_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(innovad_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('innova.conf')
