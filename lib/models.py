...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, innovad):
        golist = innovad.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_innovad(innovad, item)
    ...
    @classmethod
    def import_gobject_from_innovad(self, innovad, rec):
        import decimal
        import innovalib
        import inflection
        ...
        # shim/innovad conversion
        object_hex = innovalib.SHIM_deserialise_from_innovad(object_hex)
        objects = innovalib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from innovad...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from innovad, with every run
        ...
        # get/create, then sync payment amounts, etc. from innovad - Innovad is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(innovad)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from innovad! %s" % e)
            govobj.vote_delete(innovad)
            return (govobj, None)
    ...
    def vote_delete(self, innovad):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(innovad, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, innovad, signal, outcome):
        import innovalib
        ...
        output = innovad.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = innovalib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(innovad, signal)

    def sync_network_vote(self, innovad, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = innovad.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 innova addr, non-multisig
            if not innovalib.is_valid_innova_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Innova address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (innovalib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/InnovaDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import innovalib
        obj_data = innovalib.SHIM_serialise_for_innovad(self.serialise())
    ...
    def prepare(self, innovad):
        try:
            object_hash = innovad.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not innovalib.is_valid_innova_address(addr, config.network):
    ...
    def hash(self):
        import innovalib
        return innovalib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, innovad):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - innovad.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, innovad):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - innovad.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, innovad):
        now = int(time.time())
        return (self.created_at < (now - innovad.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, innovad):
        if self.is_expired(innovad):
            return False

        return True

    def is_deletable(self, innovad):
        if self.is_expired(innovad):
            return True
